window.onload=function(){
    ...
    moden1.onclick=function(){
        time=80;
        moden1.style.color="black";
        moden2.style.color="green";
    ...
    moden2.onclick=function(){
        time=300;
        moden1.style.color="green";
        moden2.style.color="black";
    ...
    var dropFood=function(){        
        ...
        document.getElementById(x+'_'+y).style.background='url(./images/2.png)';
    ...
    var drawSnake=function(){
        for(var i=0;i<snake.length;i++){
            document.getElementById(snake[i].x+'_'+snake[i].y).style.background='green';
        }
    }
    ...
    var moveSnake=function(){
        ...
        if(kai){           
            if(newHead.x>NUM-1||newHead.x<0||newHead.y>NUM-1||newHead.y<0){
                mu.style.display='block';
                zimu.innerHTML='Game Over'+'得分：'+scores.innerHTML;
                end.style.display="none";
                return;
            }  
        }else{
        ...
        if(newHead.x==food.foodx&&newHead.y==food.foody){
            score++;
            scores.innerHTML=score*100;
            snake.push(newHead);
            document.getElementById(food.foodx+'_'+food.foody).style.background='green';
